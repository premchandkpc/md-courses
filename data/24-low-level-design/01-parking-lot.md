...
## Interactive: Parking Lot States
...
<div style="padding:16px;background:#0b0e14;border:1px solid #1e2a3a;border-radius:8px">
  <style>
    ...
    .state-available { background:#34d399;border-color:#22c55e }
    .state-occupied { background:#ef4444;border-color:#dc2626 }
    .state-reserved { background:#fbbf24;border-color:#f59e0b }
    .state-buttons {
      display:flex;
      gap:8px;
      justify-content:center;
  ...
  <div class="state-machine-title">Parking Spot States</div>
  <div class="state-demo">
    <div class="state-display state-available" id="spot-state">AVAILABLE</div>
    <div class="state-buttons">
      <button class="state-button" onclick="setSpotState('AVAILABLE')">Available</button>
      <button class="state-button" onclick="setSpotState('OCCUPIED')">Occupied</button>
      <button class="state-button" onclick="setSpotState('RESERVED')">Reserved</button>
    </div>
  </div>

  <script>
    const spotMap = {
      'AVAILABLE': { label: 'AVAILABLE', class: 'state-available' },
      'OCCUPIED': { label: 'OCCUPIED', class: 'state-occupied' },
      'RESERVED': { label: 'RESERVED', class: 'state-reserved' }
    };
    function setSpotState(state) {
      const display = document.getElementById('spot-state');
      const info = spotMap[state];
